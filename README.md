...
On Windows, keep TORIA running and double-click `diagnose-toria.cmd` in the same folder. Alternatively run `node scripts/doctor.mjs`. This checks local configuration presence, the local server/page, and the public OrcaRouter models endpoint **without sending your key or requesting model inference**. The sanitized report is saved to `data/connection-check.json`, ignored by Git. A successful network check does not validate the key, credits, or model. See the [Windows demo troubleshooting guide](docs/windows-demo-check.md) for the full manual check. The current suite has 43 passing tests, including three diagnostic-tool tests for secret handling, network failure, and stale pages.
